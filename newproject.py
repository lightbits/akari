...
def write_content(content, filename):
    if os.path.exists(filename):
        ans = input("file %s already exists, overwrite? [y/n]" % filename)
        if ans != "y":
            return
...
def new_project():
    ...
    if os.path.exists(name):
        ans = input("project already exists, overwrite? [y/n]")
        if ans != "y":
            return
    else:
        os.mkdir(name)

    write_content(premake4_lua, os.path.join(name, "premake4.lua"))
    write_content(game_h, os.path.join(name, "game.h"))
    write_content(game_cpp, os.path.join(name, "game.cpp"))
    write_content(main_cpp, os.path.join(name, "main.cpp"))
